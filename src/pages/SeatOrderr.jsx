...
const SeatOrder = () => {
  ...
  const movieData = useSelector(selectSelectedMovie);
  const selectedDate = useSelector(selectSelectedDate);
  const selectedTime = useSelector(selectSelectedTime);
  const selectedCity = useSelector(selectSelectedCity);
  const selectedCinema = useSelector(selectSelectedCinema);
  const reduxTicketPrice = useSelector(selectTicketPrice);

  // Create formatted date from selected date
  ...
  useEffect(() => {
    if (!movieData.id || !selectedDate || !selectedTime || !selectedCinema.id) {
      // Redirect to movie selection if data is missing
      navigate("/movie-ticketing");
    }
  }, [movieData, selectedDate, selectedTime, selectedCinema, navigate]);

  // State for seat selection
  const [selectedSeats, setSelectedSeats] = useState([]);
  const [soldSeats, setSoldSeats] = useState([]);
  const [totalPayment, setTotalPayment] = useState(0);
  const ticketPrice = reduxTicketPrice || 10; // Use Redux price or default to 10
  ...
  const handleCheckout = () => {
    // Dispatch selected seats and price to Redux store
    dispatch(
      selectSeats({
        seats: getSortedSeats(),
        price: ticketPrice,
      })
    );
                ...
                <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
                  ...
                  <div className="flex flex-col gap-2 w-full">
                    <h2 className="text-lg sm:text-xl font-semibold">
                      {movieData.title || "Movie not selected"}
                    </h2>
                    ...
                    <div className="mt-2 sm:mt-4 flex flex-col sm:flex-row sm:items-center justify-between">
                      <span className="text-xs sm:text-sm text-gray-600">
                        {selectedCinema.name || "Cinema not selected"} -{" "}
